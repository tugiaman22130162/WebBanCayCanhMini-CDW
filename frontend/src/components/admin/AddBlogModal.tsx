...
export default function AddBlogModal({ isOpen, onClose, onSuccess }: AddBlogModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Kiểm tra nội dung rỗng do Tiptap mặc định có thẻ <p></p>
        ...
        try {
            ...
            showSuccessToast("Thêm bài viết thành công!", 2000);
            setFormData({
                title: "", content: "", slug: "", readingTime: 5, type: "TIPS", published: true
            });
            setImageFile(null);
            setImagePreview("");
            onSuccess();
            ...
            showErrorToast("Có lỗi xảy ra khi thêm bài viết.", 2000);
        } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                <div className="p-5 border-b flex justify-between items-center bg-gray-50">
                    <h3 className="font-bold text-lg text-gray-800">Thêm Bài Viết Mới</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-red-500 transition-colors">
                        <span className="material-symbols-outlined">close</span>
                    </button>
                </div>
                <div className="overflow-y-auto p-6">
                    <form id="add-blog-form" onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            {imagePreview && (
                                <div className="mt-4 relative w-32 h-24 rounded-xl border border-gray-200 overflow-hidden shadow-sm group">
                                    ...
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                        <button type="button" onClick={() => { setImageFile(null); setImagePreview(""); }} className="bg-red-500 text-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-red-600 transition-colors" title="Xóa ảnh">
                ...
                <div className="p-5 border-t bg-gray-50 flex justify-end gap-3">
                    ...
                    <button type="submit" form="add-blog-form" disabled={isSubmitting} className="px-8 py-2.5 rounded-xl bg-[#006c49] text-white font-bold hover:bg-[#005236] disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center gap-2 shadow-lg">
